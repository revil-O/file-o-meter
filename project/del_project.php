...
?>
		<table cellpadding="2" cellspacing="0" border="0" width="100%">
			<tr valign="middle">
				<td class="main_table_header" width="100%"><?php get_text(381); //Projekt l&ouml;schen ?></td>
			</tr>
			...
			<tr>
				<td colspan="2" class="main_table_content">
					...
					<form method="post" action="index.php<?php echo $gv->create_get_string('?fileinc=project&amp;pid_int='.$_GET['pid_int']); ?>" accept-charset="UTF-8">
						<?php
							if ($_GET['fileinc'] == 'del_projekt')
							{
								echo '<input type="hidden" name="job_string" value="del_project" />';
							}
							else
							{
								echo '<input type="hidden" name="job_string" value="kill_project" />';
							}
						?>
						...
						<table cellpadding="2" cellspacing="0" border="0" width="100%" class="content_table">
							...
							<tr class="content_tr_1">
								<td colspan="2">
									<?php
										if ($_GET['fileinc'] == 'del_projekt')
										{
											echo '<strong>'.get_text(382, 'return').'</strong>'; //Moechten Sie wirklich dieses Projekt loeschen?
										}
										else
										{
											echo '<strong>'.get_text(383, 'return').'</strong>'; //Moechten Sie wirklich dieses Projekt inkl. dem gesamten Inhalt entgueltig loeschen?
										}
									?>
							...
							<tr class="content_tr_1">
								<td colspan="20" align="center"><input type="submit" value="<?php get_text('del');//Löschen ?>" /></td>
							</tr>
						</table>
					</form>
				</td>
