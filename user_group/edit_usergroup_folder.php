<?php
	/**
	 * edit usergroup folder/file access
	...
	{
		$sql = $cdb->select('SELECT usergroup FROM fom_user_group WHERE usergroup_id='.$_GET['ugid_int']);
		$result = $cdb->fetch_array($sql);
		$user_group_name_string = $result['usergroup'];

		$acces_array = array();
		$sort_array = array();

		$sql = $cdb->select('SELECT * FROM fom_access WHERE usergroup_id='.$_GET['ugid_int']);
...
?>
		...
		<table cellpadding="2" cellspacing="0" border="0" width="100%">
			<tr valign="middle">
				<td class="main_table_header" width="100%"><?php get_text(319);//Zugriffsrechte f&uuml;r die Benutzergruppe ?><?php echo ' "'.$user_group_name_string.'"' ?></td>
			</tr>
			...
			<tr>
				<td colspan="2" class="main_table_content">
					...
					<table cellpadding="2" cellspacing="0" border="0" width="100%" class="content_table">
						<tr>
							<td class="content_header_1" width="60%"><?php get_text('folder');//Folder ?> / <?php get_text('file');//File ?></td>
								...
								<tr class="content_tr_<?php echo $style; ?>" valign="top">
									...
									<td colspan="2">
										<table cellpadding="0" cellspacing="0" border="0" width="100%">
											<tr valign="top">
												<td width="80%">
													<form method="post" action="index.php<?php echo $gv->create_get_string('?fileinc=edit_usergroup_folder&amp;ugid_int='.$_GET['ugid_int']); ?>" accept-charset="UTF-8">
														<div style="float: left; width: 90%;">
															<input type="hidden" name="job_string" value="edit_usergroup_folder" />
															...
															?>
															<input type="hidden" name="usergroup_id_int" value="<?php echo $_GET['ugid_int']; ?>" />
															<?php
												...
												<td width="10%">
													<form method="post" action="index.php<?php echo $gv->create_get_string('?fileinc=edit_usergroup_folder&amp;ugid_int='.$_GET['ugid_int']); ?>" onsubmit="return confirm_del();" accept-charset="UTF-8">
														<input type="hidden" name="job_string" value="del_usergroup_folder" />
														<input type="hidden" name="type_string" value="<?php echo $access_data['type']; ?>" />
														<?php
															$reload->create();
						...
						<?php
							...
							if ($count == 0)
							{
								echo '<tr><td colspan="3" class="content_tr_1">'.get_text(87, 'return').'</td></tr>';//Kein Eintrag vorhanden
							}
						?>
					</table>
				</td>
